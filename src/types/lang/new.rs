...
impl LangCode {
    ...
    pub(crate) fn get_new(s: &str) -> Option<LangCode> {
        let mut code = [0u8; 3];

        return str_to_ascii_words(s).find_map(|s| {
            let len = s.len();
            if !matches!(len, 2 | 3) {
                return None;
            }
            for (dst, src) in code[..len].iter_mut().zip(s.bytes()) {
                *dst = src.to_ascii_lowercase();
            }
            MAP_FROM_BYTES.get(&code[..len]).copied()
        });
...
impl FromStr for LangCode {
    ...
    fn from_str(s: &str) -> Result<LangCode> {
        Self::get_new(s).ok_or_else(|| err!("Not found a valid language code"))
    }
}

use phf::{Map, phf_map};
static MAP_FROM_BYTES: Map<&[u8], LangCode> = phf_map! {
    b"und" => LangCode::Und,

        ...
        $(
            ...
            impl SetGetPathTrackField<$marker> for MediaInfo<'_> {
                ...
                fn try_set(&mut self, path: &Path, num: u64) -> Result<(), MuxError> {
                    let _ = self.get_mut_ti_cache(path, num).ok_or("None CacheMIOfFileTrack")?;
                    ...
                    let tic = self.get_mut_ti_cache(path, num).expect("None CacheMIOfFileTrack");
                    tic.$tic_field = state;
                    result
                }

                fn try_get(&mut self, path: &Path, num: u64) -> Result<&Self::FieldType, MuxError> {
                    let tic = self.get_mut_ti_cache(path, num).ok_or("None CacheMIOfFileTrack")?;

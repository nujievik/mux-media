...
use mux_media::{CLIArg, MCOffOnPro, OffOnPro};
...
fn test_empty() {
    let off = new(&[]);
    assert!(!off.pro);
    assert!(off.add_defaults);
    assert!(off.add_forceds);
    assert!(off.add_enableds);
...
#[test]
fn test_manual_on() {
    assert!(new(&["--add-defaults"]).add_defaults);
    assert!(new(&["--add-forceds"]).add_forceds);
    assert!(new(&["--add-enableds"]).add_enableds);
...
#[test]
fn test_manual_off() {
    assert!(!new(&["--no-add-defaults"]).add_defaults);
    assert!(!new(&["--no-add-forceds"]).add_forceds);
    assert!(!new(&["--no-add-enableds"]).add_enableds);
...
fn test_manual_on_with_pro() {
    assert!(new(&["--pro", "--add-defaults"]).add_defaults);
    assert!(new(&["--pro", "--add-forceds"]).add_forceds);
    assert!(new(&["--pro", "--add-enableds"]).add_enableds);

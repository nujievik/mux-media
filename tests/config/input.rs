...
fn iter_media_and_assert(input: &ConfigInput, dir: &PathBuf, expected: &Vec<Vec<PathBuf>>) {
    let mut cnt = 0;
...
fn collect_fonts_and_assert_eq(dir: &Path, expected: &[&str], input: &ConfigInput) {

...
use crate::{
    ...
    i18n::logs,
    markers::{MCAutoFlags, MCInput, MCTools, MIAttachsInfo, MIMkvmergeI, MISavedTracks},
};
...
impl MediaInfo<'_> {
    ...
    /// Replaces cache to default value, returning current value.
    #[inline(always)]
    ...
    #[inline(always)]
    pub fn upd_group_stem(&mut self, stem: impl Into<OsString>) {
        self.cache.of_group.stem = CacheState::Cached(stem.into());
    }

    /// Attempts to insert a `path` into the cache.
    ///
    /// Returns an error if retrieving mkvmerge info fails.
    pub fn try_insert(&mut self, path: ArcPathBuf) -> Result<(), MuxError> {
    ...
    /// - Retrieving mkvmerge info fails.
    ///
    /// # Logging
    ///
    ...
    ///
    /// - Retrieving mkvmerge info fails and `exit_on_err` is `false`.

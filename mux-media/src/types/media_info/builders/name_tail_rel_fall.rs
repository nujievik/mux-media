use crate::{
    ...
    markers::{
        MCTrackLangs, MCTrackNames, MICmnRegexWord, MICmnStem, MIPathTail, MIRelativeUpmost,
        MITICache, MITIName, MITIWordsName, MITargets, MITracksInfo, MIWordsPathTail,
...
impl MediaInfo<'_> {
    ...
    pub(crate) fn build_words_path_tail(&mut self, media: &Path) -> Result<Vec<String>, MuxError> {
        ...
        let re = immut!(@try, self, MICmnRegexWord)?;
        let tail = self.try_immut::<MIPathTail>(media)?;
    ...
    pub(crate) fn build_words_relative_upmost(
        ...
        let re = immut!(@try, self, MICmnRegexWord)?;
        let tail = self.try_immut::<MIRelativeUpmost>(media)?;
    ...
    pub(crate) fn build_ti_words_name(
        ...
        let re = immut!(@try, self, MICmnRegexWord)?;
        let name = self.try_immut_ti::<MITIName>(media, track)?;

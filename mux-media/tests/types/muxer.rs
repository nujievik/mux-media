...
#[test]
fn test_as_ext() {
    assert_eq!("avi", Muxer::AVI.as_ext());
    assert_eq!("mp4", Muxer::MP4.as_ext());
    assert_eq!("mkv", Muxer::Matroska.as_ext());
    assert_eq!("webm", Muxer::Webm.as_ext());
}
...
macro_rules! test_mux_current_any {
    ($fn:ident, $in_arg:expr, $out_arg:expr, $muxer:ident) => {
        ...
        fn $fn() {
            ...
            let media = mc
                .field::<MCInput>()
                .iter_media_grouped_by_stem()
                .next()
                .unwrap();
            media_info
                .try_insert_many_filtered(media.files, true)
                .unwrap();
